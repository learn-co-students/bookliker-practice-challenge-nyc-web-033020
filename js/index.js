...
// √ add else if statement for click event to assign event read button
  ...
  const addUserToBook = (readButton) => {
    const bookDiv = readButton.parentNode
    const usersList = bookDiv.children[3] 
    const pourosLi = document.createElement('li')
    pourosLi.textContent = userPouros.username
    pourosLi.dataset.userId = userPouros.id
    usersList.append(pourosLi)
    
    let currentUsersArray = []
    
    getUsersForBook(bookDiv.dataset.bookId).then(users=>{
        users.push(userPouros)
        fetch(`${booksUrl}/${bookDiv.dataset.bookId}`,{
            method: "PATCH",
            headers:{
...
// This is the old solution to the read button
    // const bookDiv = readButton.parentNode
    // const usersList = bookDiv.children[3] 
    // const users = []
    // for (let key in usersList.children){
    //     let newUser = {"id":usersList.children[key].dataset.userId,
    //     "username":usersList.children[key].textContent
    //     }
    //     users.push(newUser)
    // console.log(newUser)
    // debugger
